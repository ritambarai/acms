...
def gen_c_xml_parser(tables, subcategories=None, xml_map=None, always_overwrite=None,
    ...
    lines.append('#include "alert_manager.h"')
    lines.append('}')
    ...
    for t, fields in tables.items():
        ...
        if subcats:
            ...
            if t in xml_map:
                ...
                lines.append(f"  if (content.length() > 0) {{")
                lines.append(f"    n = parse_{tl}_xml(content.c_str(), {args});")
                lines.append(f"  }} else {{")
            ...
            if t in xml_map:
                ...
                lines.append(f"  int n;")
                lines.append(f"  if (_content.length() > 0) {{")
                lines.append(f"    n = parse_{tl}_xml(_content.c_str(), &{tl}_table);")
                lines.append(f"  }} else {{")
                lines.append(f'    Serial.println("[XML] {t}.xml empty/missing -- using default");')
    ...
    if xml_map and "Metadata" in xml_map:
        ...
        if op_entries:
            ...
            lines.append("        add_alert_queue((uint16_t)var_pool_id, (uint16_t)_cid);")
            lines.append("      }")
            lines.append("    }")
