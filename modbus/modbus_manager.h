 ...
 * Automatically cycles through every row in variables_modbus_table:
 * sends a query, waits for the response, prints the result, then
 * moves to the next row.  Also handles slave-mode queries when no
 * table row query is pending. */
